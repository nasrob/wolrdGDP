...
public class CountryDao {
	...
	private static final String SELECT_CLAUSE = "SELECT" + " c.Code, " + " c.Name, "
			+ " c.Continent, "
			+ " c.region, "
	...
	public List<Country> getCountries(Map<String, Object> params) {
		...
		return namedParamJdbcTemplate.query(SELECT_CLAUSE
				+ " WHERE 1 = 1 "
				+ (!params.get("search").toString().isEmpty()
					? SEARCH_WHERE_CLAUSE : "")
				+ (!params.get("continent").toString().isEmpty()
						? CONTINENT_WHERE_CLAUSE : "")
				+ (!params.get("region").toString().isEmpty()
						? REGION_WHERE_CLAUSE : "")
				+ PAGINATION_CLAUSE,
